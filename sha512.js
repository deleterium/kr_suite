...
class Sha512 {
    ...
    /**
     * Generates SHA-512 hash of byte array.
     *
     ...
     *   const hash = Sha512.hash([ 0x0a, 0xdd, 0xfe]); // 'hash = "e234e8796cb588f64bedfeb4b84152d246b5601d8245b19da765ad4efb3fee8f4437f4873d5e8605b724d0f520621fe18c2e8e7bf0a93ef18bbf2ffcd3c0cb29"
     */
    static hash(msg) {
...
//        msg += String.fromCharCode(0x80);  // add trailing '1' bit (+ 0's padding) to string [§5.1.2]
        ...
        for (let i=0; i<N; i++) {
            ...
            for (let j=0; j<16; j++) { // encode 8 chars per uint64 (128 per block), big-endian encoding
                const lo = (byte_array[i*128+j*8+0]<<24) | (byte_array[i*128+j*8+1]<<16)
                         | (byte_array[i*128+j*8+2]<< 8) | (byte_array[i*128+j*8+3]<< 0);
                const hi = (byte_array[i*128+j*8+4]<<24) | (byte_array[i*128+j*8+5]<<16)
                         | (byte_array[i*128+j*8+6]<< 8) | (byte_array[i*128+j*8+7]<< 0);
        ...
        const lenHi = ((byte_array.length-1)*8) / Math.pow(2, 32);
        const lenLo = ((byte_array.length-1)*8) >>> 0; // note '>>> 0' coerces number to unsigned 32-bit integer
